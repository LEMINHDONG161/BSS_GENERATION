...
File_name = ['Target_R3']; % Save name

%% 선박(함정) 설정
% <선박용 디젤 엔진 (Ref. H4D, 현대기계공업)>
% 어선, 화물선, 작업선, 유람선, 여객선 등 사용
% 3target_1
 ERPM = 8500; % Engine RPM
 Engine_type = 2; % 2: 2행정, 4: 4행정
 NOC = 2; % 실린더 개수
 NOB = 2; % Blade 개수 (프로펠러 날개)
 Rg = 1.5; % RPM 기어비 (프로펠러 축 기어잇수 / 엔진 크랭크 축 기어잇수)
...
%===== 내부 엔진 (LOFAR) =====%
Amp_CSR = [2.5 2]; % Crnk Shaft rate
Amp_CFR = [5 2]; % Cylinder Firing Rate
Amp_ERF = [2]; % Engine Firing Rate

%===== 외부 프로펠러 (DEMON) =====%
Amp_PSR = [0.56 0.67 0.82]; % Propeller Shaft Rate
Amp_BR = [0.3]; % Blade Rate
...
% Transient Signal
sig_TS = zeros(size(t));
sig_TS =  sig_TS + 4*(cos(2*pi*250*t).*heaviside(t).*heaviside(50-t)+cos(2*pi*250*t).*heaviside(t-70).*heaviside(180-t));

sig_TS1 = zeros(size(t));
sig_TS1 =  sig_TS1 + 4*(cos(2*pi*170*t).*heaviside(t).*heaviside(50-t)+cos(2*pi*170*t).*heaviside(t-60).*heaviside(180-t));
...
% 표적 신호
Target_sig = S_un+S_mn+S_ub+S_mb;
