...
File_name = ['Target_N1_interup']; % Save name
...
% 3target_1
 ...
 Rg = 2.92; % RPM 기어비 (프로펠러 축 기어잇수 / 엔진 크랭크 축 기어잇수)
...
%===== 내부 엔진 (LOFAR) =====%
Amp_CSR = [2 5]; % Crnk Shaft rate
Amp_CFR = [2.5 2]; % Cylinder Firing Rate
Amp_ERF = [4]; % Engine Firing Rate

%===== 외부 프로펠러 (DEMON) =====%
Amp_PSR = [ 0.36 0.27 0.12]; % Propeller Shaft Rate
Amp_BR = [0.23]; % Blade Rate
...
% Transient Signal
sig_TS = zeros(size(t));
sig_TS =  sig_TS + 4*(cos(2*pi*250*t).*heaviside(t).*heaviside(30-t)+cos(2*pi*250*t).*heaviside(t-40).*heaviside(180-t));
...
% 표적 신호
Target_sig = S_un+S_mn+S_ub+S_mb+sig_TS;
