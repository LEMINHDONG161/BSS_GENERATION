...
File_name = ['Target_FDJADE_extinction']; % Save name
...
NOC = 4; % 실린더 개수
NOB = 4; % Blade 개수 (프로펠러 날개)
Rg = 3.02; % RPM 기어비 (프로펠러 축 기어잇수 / 엔진 크랭크 축 기어잇수)
...
% 비변조 협대역 신호
sig_CSR = sig_CSR.*heaviside(t).*heaviside(100-t);
sig_ERF = sig_ERF.*heaviside(t).*heaviside(120-t);
...
% Transient Signal
sig_TS = zeros(size(t));
sig_TS =  sig_TS + 4*cos(2*pi*300*t).*(sin(2*pi*1/40*t + pi/2).*heaviside(t-30).*heaviside(50-t)+sin(2*pi*1/20*t).*heaviside(t-120).*heaviside(130-t));
